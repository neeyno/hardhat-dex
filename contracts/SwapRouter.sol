...
contract SwapRouter is SwapRouterInternal {
    ...
    */
    ...
    function addLiquidity(
        ...
        liquidity = ISwapPair(pairAddress).mint(to);
    }
    ...
    function removeLiquidity(
        ...
        if (amountA < amountAMin) revert SwapRouter__InsufficientAAmount();
        if (amountB < amountBMin) revert SwapRouter__InsufficientBAmount();
    }
    ...
    function swapExactTokensForTokens(
        ...
        if (amounts[amounts.length - 1] < amountOutMin)
            revert SwapRouter__InsufficientOutputAmount();

        _safeTransferFrom(
            path[0],
            msg.sender,
            SwapLibrary.pairFor(address(factory), path[0], path[1]),
            amounts[0]
        );
    ...
    function swapTokensForExactTokens(
        ...
        amounts = SwapLibrary.getAmountsIn(address(factory), amountOut, path);
        if (amounts[amounts.length - 1] > amountInMax)
            revert SwapRouter__ExcessiveInputAmount();
    ...
    function _swap(
        ...
        for (uint256 i; i < path.length - 1; i++) {
            ...
            address to = i < path.length - 2
                ? SwapLibrary.pairFor(address(factory), output, path[i + 2])
                : to_;

            ISwapPair(SwapLibrary.pairFor(address(factory), input, output))
        ...
        if (reserveA == 0 && reserveB == 0) {
            ...
            if (amountBOptimal <= amountBDesired) {
                if (amountBOptimal <= amountBMin)
                    revert SwapRouter__InsufficientBAmount();
                ...
                if (amountAOptimal <= amountAMin)
                    revert SwapRouter__InsufficientAAmount();
    ...
    function _safeTransferFrom(
        ...
        if (!success || (data.length != 0 && !abi.decode(data, (bool))))
            revert SwapRouter__SafeTransferFailed();
    }
}
